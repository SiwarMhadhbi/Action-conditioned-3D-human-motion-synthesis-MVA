...
def main(args):
    ...
    if not os.path.isfile(os.path.join(datapath,video_file)):
        exit(f'Input video \"{video_file}\" does not exist!')

    output_path = os.path.join(args.output_folder, os.path.basename(video_file).replace('avi', 'pkl'))
    #os.makedirs(output_path, exist_ok=True)

    image_folder, num_frames, img_shape = video_to_images(os.path.join(datapath,video_file), return_info=True)
    ...
    joblib.dump(vibe_results, os.path.join(output_path))
