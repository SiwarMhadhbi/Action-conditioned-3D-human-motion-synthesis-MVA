...
datapath =  '/content/drive/MyDrive/Project_Rec_Vis/data/ntu13/ntu13_rgb'
...
def main(args):
    ...
    if demo_mode == 'video':
        ...
        if not os.path.isfile(datapath+'/'+video_file):
            exit(f'Input video \"{video_file}\" does not exist!')

        output_path = os.path.join(args.output_folder, os.path.basename(video_file).replace('avi', 'pkl'))
        output_path1 = args.output_folder
        #os.makedirs(output_path, exist_ok=True)

        if os.path.isdir(os.path.join(output_path1, 'tmp_images')):
            input_image_folder = os.path.join(output_path1, 'tmp_images')
            ...
            input_image_folder, num_frames, img_shape = video_to_images(
                os.path.join(datapath,video_file),
                img_folder=os.path.join(output_path1, 'tmp_images'),
                return_info=True
            )
    ...
    """logger.add(
        os.path.join(output_path1, 'demo.log'),
        level='INFO',
        colorize=False,
    )
    ...
    if args.mode == 'video':
        ...
        if not args.no_save:
            ...
            joblib.dump(pare_results,output_path)
            shutil.rmtree('/content/drive/MyDrive/Project_Rec_Vis/data/ntu13/outputPARE/tmp_images', ignore_errors=True)
